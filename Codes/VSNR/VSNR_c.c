  ...
  double PhiStar(fftw_complex *lambda, int taille, double p, double C, double alpha){
	...
	/*mexPrintf("i:%i -- norme : %1.5e -- lambdaAbs:%1.5e -- lambda[i][0]:%1.5e -- lambda[i][1]:%1.5e\n",i,norme,lambdaAbs,lambda[i][0],lambda[i][1]);*/
      }
      norme*=alpha;
    }
    /* pour la norme infinie, on utilise p=3 pour pouvoir avoir un pointeur sur un double*/
  ...
  void mexFunction( int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]) {
    ...
    plhs[1] = mxCreateDoubleMatrix(nit+1,1 , mxREAL);
    plhs[2] = mxCreateDoubleMatrix(nit+1,1 , mxREAL);
    plhs[3] = mxCreateDoubleMatrix(nit+1,1 , mxREAL);
